...
| 21 | Rename all 5 workers `pwn4ge*` → `pwn4g3*` (+ URLs, CSP, docs) | `workers/*/wrangler.jsonc`, `workers/*/src/index.ts`, `src/lib/config.ts`, `src/lib/components/Signal.svelte`, `src/app.html`, `_headers`, `src/routes/+page.svelte`, `workers/r2-cors.json`, `README.md`, `AGENTS.md` | Otis | in_progress |
## Notes / decisions
